namespace Unit_3___Linq_2
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // RunExampleOne(students);
            // RunExampleTwo(students);
            // RunExampleThree(students);
            // RunExampleFour(students);
            RunExampleFive(GetNames());
        }


        // Select is a Mapping Method, it takes A -> B
        // for All elements in your collection 
        // Where A is a value and B is a new Transformed values
        // that is "derived" from A
        static void RunExampleOne(List<string> students)
        {

            // Deferred Execution -> Lazy Evaluation
            // Deferred execution just means some Linq methods WILL NOT RUN
            // unless it's told to run by converting your ling "statement" into a collection
            // or simply for each looping over it
            List<string> studentsAllCaps = students.Select(student => student.ToUpper()).ToList();
            // if we DO NOT use ToList(), we are actually just STORING the "IDEA" of something
            // being able to loop
        }

        // Where is a filtering method, it takes A -?> A
        // for All elements in your collection
        // where A is a value we predicate to see IF
        // we want to KEEP A in our link statment, IE.
        // we are FILTERING to keep whatever we want 
        // based on our boolean expression
        static void RunExampleTwo(List<string> students)
        {
            //   if the boolean expression in your LINQ method evaluates to TRUE, then said element 
            //  will be KEPT for whomever calls this linq statement. in this case, if the students
            // name Length is less than and equal to 5, then we keep that name, all other names will
            // be FILTERED OUT, we don't keep them in our new LIST
            List<string> studentsWithShortNames = students.Where(student => student.Length <= 5).ToList();
        }


        // you can combine Linq Methods together to make a sort of "Query"
        // combining these methods allows Deffered Execution to execute EACH
        // Linq Method in ORDER PER ELEMENT
        // so for each ELEMENT we will run the linq methods in order
        static void RunExampleThree(List<string> students)
        {
            // var type        var name
            List<string> capatalizedShortStudentNames = students
                .Select(student => student.ToUpper())
                .Where(student => student.Length <= 5)
                .ToList(); // ToList() cuased select -> where actually loop

        }



        // There are many "Executing" methods you can call to force your linq
        // "functions" to loop over your collections
        static void RunExampleFour(List<string> students)
        {
            // method chaining works because a Method will RETURN a type, THEN you are 
            // are simply CALLING a different Method on that RETURNED TYPE

            bool hasJ = "   James "
 /* "     James  "*/.Trim() // Trim returns a STRING, we can then call ToLower on that returned string!
       /* "James" */.ToLower() // this method is ACTUALLY being called on the RETURN type from the above method.
       /* "james" */.Replace('m', 'n') // Replace is being Called on the methord return type above
         /* janes */.Contains('J'); // false
            ...
            // IEnumerable means this variable CAN be looped over.
            IEnumerable<string> capatalizedShortStudentNames = students
                .Select(student => student.ToUpper())
                .Where(student => student.Length <= 5);


            // Count will force a loop to happen, it will Count each
            // element in your IEnumerable<T>
            int numberOfStudents = capatalizedShortStudentNames.Count();

            
            IEnumerable<int> nameLengths = capatalizedShortStudentNames
                .Select(student => student.Length);

            // loops over each elements and get the highest VALUE
            int nameWithLongestLength = nameLengths.Max();

            // loops over each elements and gets the LOWEST VALUE
            int nameWithShortestLength = nameLengths.Min();

            // loops over each elements and gets the SUM of all elements
            int sumOfNameLengths = nameLengths.Sum();

            // loops over each elemnts and gets the AVERAGE of all elements
            double averageOfLengths = nameLengths.Average();
        }

        // Query Syntax is an alternate way to write LINQ
        // it looks like sql, but under the hood, it's just
        // calling the methods above... just an alternate 
        // SYNTANX for writing LINQ IF you feel more comfortable
        // with SQL (the language)
        static void RunExampleFour(IEnumerable<string> students)
        {
            // this Query syntax is LITERALLY doing the SAME things
            // as my above examples, the syntax is just my SQL Like.
            List<string>  someStudents = (from student in students
                                        where student.Length <= 5
                                        orderby student descending
                                        select student.ToUpper()
                                        ).ToList();

            // this is using the actual methods... just normal coding....
            // the above gets compiled into the method calls below
        ...
        // Example "App" where the user Filters down
        // to the names they want
        static void RunExampleFive(IEnumerable<string> students)
        {
            Console.WriteLine("Welcome to my student filter / add app!");
            foreach (var student in students)
            {
                Console.WriteLine(student);
            }

            Console.Write("Provide a filter please... ");
            string filter = Console
                               .ReadLine() // "    Frank    "
                               .Trim()     // "Frank"
                               .ToUpper(); // "FRANK"

            IEnumerable<string> filteredNames = students
                .Where(student => student.ToUpper().Contains(filter));

            Console.WriteLine("Heres you list of names sire!");
            Console.WriteLine();
            ...
            int filteredNamesCount = filteredNames.Count();
            int allNamesCount = students.Count();

            Console.WriteLine();
            Console.WriteLine($"There are {filteredNamesCount} \"{filter.Trim()}\"s out of {allNamesCount} students");
        }
